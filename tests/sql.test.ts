...
beforeEach(async () => {
    db = await createDB({username: 'root', logLevel: 'LOG', deleteDBAfterStopped: !!process.env.CI, dbPath: process.env.CI ? `${process.cwd()}/dbs/${process.env.JEST_WORKER_ID}` : undefined})
})
