...
for (const version of !process.env.VERSION_REQUIREMENT ? DOWNLOADABLE_MYSQL_VERSIONS : DOWNLOADABLE_MYSQL_VERSIONS.filter(v => satisfies(v, process.env.VERSISON_REQUIREMENT || 'error'))) {
    try {
