        ...
        if (stderr && !stderr.includes('is created with an empty password')) {
            ...
            if (process.platform === 'linux' && stderr.includes('libaio.so')) {
                ...
                if (binary.installedOnSystem) {
                    throw 'libaio could not be found while running system-installed MySQL. libaio must be installed on this system for MySQL to run. To learn more, please check out https://github.com/Sebastian-Webster/mysql-memory-server-nodejs/blob/v1.16.0/docs/SUPPORTED_MYSQL_DOWNLOADS.md#required-dependencies'
                }
                ...
                throw 'You do not have libaio1 installed. Please install the libaio1 package for the downloaded MySQL binary to run. Learn more here: https://github.com/Sebastian-Webster/mysql-memory-server-nodejs/blob/v1.16.0/docs/SUPPORTED_MYSQL_DOWNLOADS.md#required-dependencies'
            }
            throw stderr
        }
