...
function getBinaryURL(versionToGet = "x", currentArch) {
    ...
    if (!(0, TypeCheckers_1.isSupportedOS)(currentOS))
    ...
    if (currentOS === 'win32' && currentArch === 'arm64')
        throw 'mysql-memory-server has detected you are running Windows on ARM. MySQL does not support Windows on ARM. To get this package working, please try setting the "arch" option to "x64" to run the x64 version of MySQL instead.';
    ...
    if (process.platform === 'linux') {
        if (LinuxOSRelease_1.default.NAME === 'Ubuntu' && typeof LinuxOSRelease_1.default.VERSION_ID === 'string' && LinuxOSRelease_1.default.VERSION_ID >= '24.04') {
            ...
            selectedVersions = selectedVersions.filter(v => (0, semver_1.satisfies)(v, constants_1.MYSQL_ALPINE_VERSION_RANGE));
            if (selectedVersions.length === 0) {
                throw `mysql-memory-server has detected you are running this package on Alpine Linux. The source for MySQL with musl libc only provides binaries for the following version ranges: "${constants_1.MYSQL_ALPINE_VERSION_RANGE}" and as such only those versions can be used with this package. Please use a version within those ranges.`;
            }
        }
    }
